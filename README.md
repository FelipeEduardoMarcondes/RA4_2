...
(
    ...
    (
        ((FAT N *) FAT)    # FAT = FAT * N
        ((N 1 +) N)        # N = N + 1
        +                  # Combina (retorna soma, mas não importa)
    )
    while
)
(FAT)
...
### 14.8 Exemplo 8: Série de Taylor para Cosseno
...
**Saída Esperada:** ~0.875 (com perda de precisão em Q8.8)
...
Este compilador implementa **Q8.8 (ponto fixo)** pelas seguintes razões:
